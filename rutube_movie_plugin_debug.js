    ...
    (function () {
        ...
        function cacheRequest(movie, isTv, success, fail) {
            ...
            try {
                ...
                var query = cleanString([search, year, isTv ? 'сериал' : 'фильм'].join(' ')); // Упрощённый запрос
                            ...
                            if (data && data.results && data.results.length) {
                                ...
                                var getRate = function(r){
                                    if (r._rate === -1) {
                                        ...
                                        r._rate -= wordDiff * 50;
                                        r._rate += r.duration > 3600 ? 100 : 0; // Даём бонус за длительность, но не штрафуем короткие видео
                                    }
                                    return r._rate;
                                };
                                results = data.results.filter(function(r){
                                    ...
                                    return r.embed_url && // Убедимся, что есть URL для воспроизведения
                                        !r.is_hidden && !r.is_deleted && !r.is_locked && !r.is_audio && !r.is_paid && !r.is_livestream && !r.is_adult && // Базовые фильтры
                                        getRate(r) > 100; // Пониженный порог
                                }).sort(function(a,b){
