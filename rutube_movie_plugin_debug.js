    ...
    (function () {
        ...
        function cacheRequest(movie, isTv, success, fail) {
            ...
            try {
                ...
                var query = cleanString([search, year, isTv ? 'сериал' : 'фильм'].join(' '));
                    ...
                    network.native(
                        ...
                        function (data) {
                            ...
                            if (data && data.results && data.results.length) {
                                ...
                                queryWord.push(isTv ? 'сериал' : 'фильм');
                            ...
                            if (results.length) {
                                ...
                                // Добавляем тестовый плейлист, чтобы проверить воспроизведение
                                var testPlaylist = [{
                                    title: 'Test Video - Big Buck Bunny',
                                    url: 'https://rutube.ru/play/embed/3e6f5a1b2c3d4e5f6a7b8c9d0e1f2a3b',
                                    iptv: true
                                }];
                                console.log('RuTube Plugin: Using test playlist:', testPlaylist);
                                typeof success === 'function' && success.apply(this, [testPlaylist]);
                            }
                            network.clear();
                            network = null;
                        },
                        function (data) {
                            ...
                            if (!proxy && !window.AndroidJS && data && 'status' in data && 'readyState' in data && data.status === 0 && data.readyState === 0) {
                                ...
                                // Добавляем тестовый плейлист в случае ошибки сети
                                var testPlaylist = [{
                                    title: 'Test Video - Big Buck Bunny',
                                    url: 'https://rutube.ru/play/embed/3e6f5a1b2c3d4e5f6a7b8c9d0e1f2a3b',
                                    iptv: true
                                }];
                                console.log('RuTube Plugin: Using test playlist due to network error:', testPlaylist);
                                typeof success === 'function' && success.apply(this, [testPlaylist]);
                            }
                            network.clear();
                            network = null;
