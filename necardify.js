(function () {
  ...
  var Trailer = /*#__PURE__*/function () {
    function Trailer(object, video, ratings) {
      var _this = this;
      ...
      this.object = object;
      this.video = video;
      this.ratings = ratings; // Сохраняем рейтинги (TMDB, IMDb, KP)
      this.player;
    ...
    _createClass(Trailer, [{
      ...
      value: function renderRating() {
        console.log('Cardify: Рендеринг рейтингов...');
        var rateLine = this.object.activity.render().find('.full-start-new__rate-line');
        rateLine.empty(); // Очищаем контейнер

        // Функция для генерации HTML рейтинга с звездами
        var generateRatingHtml = function(rating, source) {
          if (rating && typeof rating === 'number' && rating >= 0 && rating <= 10) {
            var ratingText = rating.toFixed(1);
            var stars = Math.round(rating / 2); // Преобразуем в 5-балльную шкалу
            var starsHtml = '';
            for (var i = 0; i < 5; i++) {
              starsHtml += i < stars ? 
            ...
            return `<div class="full-start__rate rate--${source.toLowerCase()}">
                      <div>${ratingText}</div>
        ...
        // Рендерим рейтинги для TMDB, IMDb, KP
        var tmdbHtml = generateRatingHtml(this.ratings.tmdb, 'TMDB');
        var imdbHtml = generateRatingHtml(this.ratings.imdb, 'IMDB');
        var kpHtml = generateRatingHtml(this.ratings.kp, 'KP');

        rateLine.append(tmdbHtml + imdbHtml + kpHtml);
      ...
      value: function start() {
        ...
        this.player.listener.follow('loaded', function () {
          console.log('Cardify: Плеер загружен, рендеринг превью и рейтингов...');
          _this4.preview();
          _this4.renderRating(); // Рендерим рейтинги
          _this4.state.start();
        });
  ...
  function startPlugin() {
    ...
    Follow.get(Type.de([102, 117, 108, 108]), function (e) {
      if (Type.co(e)) {
        ...
        // Извлекаем рейтинги из данных
        var ratings = {
          tmdb: e.data.vote_average || null, // TMDB рейтинг
          imdb: e.data.imdb_rating || null,  // IMDb рейтинг (замени на правильное поле, если нужно)
          kp: e.data.kp_rating || null       // Кинопоиск рейтинг (замени на правильное поле, если нужно)
        };
        console.log('Cardify: Рейтинги:', ratings);

        if (Main.cases().Manifest.app_digital >= 220) {
          if (Main.cases().Activity.active().activity === e.object.activity) {
            if (trailer) {
              new Trailer(e.object, trailer, ratings);
            } else {
              console.log('Cardify: Трейлер не найден, рендеринг только рейтингов...');
              var dummyTrailer = { id: '', img: '' }; // Заглушка для трейлера
              var tempTrailer = new Trailer(e.object, dummyTrailer, ratings);
              tempTrailer.renderRating(); // Рендерим только рейтинги
            }
          } else {
