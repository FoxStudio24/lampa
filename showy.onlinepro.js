  ...
  function component(object) {
    ...
    this.rch = function (json, noreset) {
      ...
      if (typeof signalR == "undefined") {
        Lampa.Utils.putScript(
          ["{localhost}/signalr-6.0.25_es5.js"],
          function () {},
          false,
          function () { load(); },
          true
    ...
    this.request = function (url) {
      ...
      if (number_of_requests < 10) {
        network["native"](url, this.parse.bind(this), this.doesNotAnswer.bind(this), false, { dataType: "text" });
    ...
    this.getEpisodes = function (season, call) {
      ...
      if (typeof object.movie.id == "number" && object.movie.name) {
        var tmdburl = "tv/" + object.movie.id + "/season/" + season + "?api_key=" + Lampa.TMDB.key() + "&language=ru";
    ...
    this.updateWatched = function () {
      ...
      if (watched) {
        ...
        if (watched.season) line.push("Сезон " + watched.season);
        if (watched.episode) line.push("Эпизод " + watched.episode);
    ...
    this.draw = function (items) {
      ...
      this.getEpisodes(items[0].season, function (episodes) {
        ...
        items.forEach(function (element, index) {
          ...
          var voice_name = choice.voice_name || (filter_find.voice[0] ? filter_find.voice[0].title : false) ||
            element.voice_name || (serial ? "Неизвестно" : element.text) || "Неизвестно";
          if (element.quality) {
          ...
          var hash_timeline = Lampa.Utils.hash(
            element.season ? [element.season, element.season > 10 ? ":" : "", element.episode, object.movie.original_title].join("")
            : object.movie.original_title
          );
          var hash_behold = Lampa.Utils.hash(
            element.season ? [element.season, element.season > 10 ? ":" : "", element.episode, object.movie.original_title, element.voice_name].join("")
        ...
        if (serial && episodes.length > items.length && !params.similars) {
          ...
          left.forEach(function (episode) {
            ...
            html.find(".online-prestige__timeline").append(
              Lampa.Timeline.render(Lampa.Timeline.view(Lampa.Utils.hash([season, episode.episode_number, object.movie.original_title].join(""))))
            );
            var img = html.find("img")[0];
      ...
      params.html.on("hover:long", function () {
        function show(extra) {
          ...
          if (Lampa.Platform.is("webos")) {
            menu.push({ title: "Запуск - Webos", player: "webos" });
          }
          if (Lampa.Platform.is("android")) {
            menu.push({ title: "Запуск - Android", player: "android" });
          }
          menu.push({ title: "Запуск - Lampa", player: "lampa" });
          menu.push({ title: "Видео", separator: true });
          menu.push({ title: "Пометить", mark: true });
          menu.push({ title: "Снять пометку", unmark: true });
          menu.push({ title: "Сбросить время", timeclear: true });
          if (extra) {
            menu.push({ title: "Скопировать ссылку", copylink: true });
          }
          menu.push({ title: "Еще", separator: true });
          if (Lampa.Account.logged() && params.element && typeof params.element.season !== "undefined" && params.element.translate_voice) {
            menu.push({ title: "Подписаться на перевод", subscribe: true });
          }
          menu.push({ title: "Очистить все метки", clearallmark: true });
          menu.push({ title: "Очистить все тайм-коды", timeclearall: true });
          Lampa.Select.show({
            title: "Действие",
            items: menu,
...
ज़                }
              ...
              if (a.subscribe) {
                Lampa.Account.subscribeToTranslation({
                  ...
                  Lampa.Noty.show("Вы успешно подписались");
                }, function () {
                  Lampa.Noty.show("Возникла ошибка");
                });
              }
            },
          });
        ...
        if (Lampa.Helper) Lampa.Helper.show("online_file", "Удерживайте клавишу 'ОК' для вызова контекстного меню", params.html);
      });
    };
    ...
    this.empty = function () {
      ...
      html.find(".online-empty__time").text("Попробуйте другой источник");
      scroll.clear();
      scroll.append(html);
      this.loading(false);
    };
    ...
    this.noConnectToServer = function (er) {
      var html = Lampa.Template.get("lampac_does_not_answer", {});
      html.find(".online-empty__buttons").remove();
      html.find(".online-empty__title").text("Ошибка");
      html.find(".online-empty__time").text(er && er.accsdb ? er.msg : "Поиск на " + sources[balanser].name + " не дал результатов");
      scroll.clear();
      scroll.append(html);
      this.loading(false);
    };
  ...
  function startPlugin() {
    ...
    Lampa.Lang.add({
      ...
      lampac_balanser_dont_work: { ru: "Поиск на ({balanser}) не дал результатов" },
      lampac_balanser_timeout: { ru: 'Источник будет переключен автоматически через <span class="timeout">10</span> секунд.' },
      lampac_does_not_answer_text: { ru: "Поиск на ({balanser}) не дал результатов" },
    });
    ...
    function resetTemplates() {
      ...
      Lampa.Template.add("lampac_does_not_answer", `
        <div class="online-empty">
          <div class="online-empty__title">Поиск на ({balanser}) не дал результатов</div>
          <div class="online-empty__time">Источник будет переключен автоматически через <span class="timeout">10</span> секунд.</div>
          <div class="online-empty__buttons">
            <div class="online-empty__button selector cancel">Отмена</div>
            <div class="online-empty__button selector change">Изменить балансер</div>
          </div>
          <div class="online-empty__templates">
            <div class="online-empty-template">
              <div class="online-empty-template__ico"></div>
        ...
        <div class="online-prestige online-prestige--folder selector">
          <div class="online-prestige__folder">
            <svg viewBox="0 0 128 128">
              <path fill="currentColor" d="M112,16H47.1A8.1,8.1,0,0,0,39,24V32H16.1A8.1,8.1,0,0,0,8,40V88a8.1,8.1,0,0,0,8.1,8H39v8a8.1,8.1,0,0,0,8.1,8H112a8.1,8.1,0,0,0,8-8V24A8.1,8.1,0,0,0,112,16ZM39,88H16V40H39ZM47,24H112v8H47.1V24Zm0,80V96H112v8Z"/>
            </svg>
          </div>
          <div class="online-prestige__body">
            <div class="online-prestige__head">
        ...
        <div class="online-prestige-watched selector">
          <div class="online-prestige-watched__icon">
            <svg viewBox="0 0 24 24">
              <path fill="currentColor" d="M12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,12,20Zm4.7-9.29a1,1,0,0,0-1.41.13L12,14.59l-1.29-1.3a1,1,0,0,0-1.42,1.42l2,2a1,1,0,0,0,1.42,0l4-4A1,1,0,0,0,16.7,10.71Z"/>
            </svg>
          </div>
