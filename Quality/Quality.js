(function () {
  ...
  setTimeout(function() {
      try {
          var installed = Lampa.Plugins.get ? Lampa.Plugins.get() : [];
          isConflict = installed.some(function(p) {
              return (p.url || '').indexOf('applecation.min.js') >= 0 || (p.name || '').indexOf('applecation') >= 0;
          });
      } catch (e) {}
  }, 3000);
  ...
  function getBest(results) {
    ...
    var limit = Math.min(results.length, 20); // Увеличил до 20 для точности
    for (var i = 0; i < limit; i++) {
      var item = results[i];
      var title = (item.Title || '').toLowerCase();

      // 1. ПРОВЕРКА ПО НАЗВАНИЮ (если ffprobe еще не прогрузился)
      var foundRes = null;
  ...
  Lampa.Listener.follow('full', function(e) {
    if (isConflict || e.type !== 'complite') return;
  ...
  var style = '<style>\
    .quality-badges-container { display: flex; gap: 0.35em; margin: 0.8em 0; min-height: 1.1em; pointer-events: none; }\
    .quality-badge { height: 1.0em; opacity: 0; transform: translateY(8px); animation: qb_in 0.4s ease forwards; }\
    .card-quality-badges { position: absolute; top: 0.3em; right: 0.3em; display: flex; flex-direction: row; gap: 0.2em; pointer-events: none; }\
    .card-quality-badge { height: 0.7em; opacity: 0; transform: translateY(5px); animation: qb_in 0.3s ease forwards; }\
